...
while [ $# -ne 0 ]; do
	...
	elif [ "$1" = "-dfrc" ] || [ "$1" = "-DFRC" ] || [ "$1" = "--dockerfile_run_command" ] || [ "$1" = "--DOCKERFILE_RUN_COMMAND" ]; then
		shift
		if [ $# -eq 0 ]; then
			PRNERR "\"--dockerfile_run_command(-dfrc)\" option is specified without parameter."
			exit 1
		fi
		OPT_DOCKERFILE_RUN_COMMAND="$1"
...
#
# Infomration about Docker run command
#
if [ -n "${OPT_DOCKERFILE_RUN_COMMAND}" ]; then
	DOCKERFILE_RUN_COMMAND="${OPT_DOCKERFILE_RUN_COMMAND}"
elif [ -n "${ENV_DOCKERFILE_RUN_COMMAND}" ]; then
	DOCKERFILE_RUN_COMMAND="${ENV_DOCKERFILE_RUN_COMMAND}"
else
	DOCKERFILE_RUN_COMMAND="${CI_DOCKERFILE_RUN_COMMAND}"
fi
...
if ! sed -e "s#%%GIT_DOMAIN_URL%%#${CI_GIT_URL}#g"							\
		...
		-e "s#%%DOCKERFILE_RUN_COMMAND%%#${CI_DOCKERFILE_RUN_COMMAND}#g"						\
...
for ONE_IMAGE_NAME in ${CI_IMAGE_NAMES}; do
	...
	if [ "${CI_DEFAULT_IMAGE_TAGGING}" -eq 1 ]; then
		...
		if ({ RUNCMD docker image build -f "${SRCTOP}/${DOCKER_FILE}" -t "${CI_DOCKER_HUB_ORG}/${ONE_IMAGE_NAME}:${CI_IMAGE_VERSION}${CI_DOCKER_IMAGE_OSTYPE_TAG}" -t "${CI_DOCKER_HUB_ORG}/${ONE_IMAGE_NAME}:latest${CI_DOCKER_IMAGE_OSTYPE_TAG}" . || echo > "${PIPEFAILURE_FILE}"; } | sed -e 's/^/    /g') && rm "${PIPEFAILURE_FILE}" >/dev/null 2>&1; then
			PRNERR "Failed to build image : ${CI_DOCKER_HUB_ORG}/${ONE_IMAGE_NAME}:${CI_IMAGE_VERSION}${CI_DOCKER_IMAGE_OSTYPE_TAG}, ${CI_DOCKER_HUB_ORG}/${ONE_IMAGE_NAME}:latest${CI_DOCKER_IMAGE_OSTYPE_TAG} from ${ONE_IMAGE_NAME}"
			exit 1
		fi
	fi
