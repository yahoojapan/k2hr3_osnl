...
if [ -z "${CI_DOCKER_IMAGE_OSTYPE}" ]; then
	...
	PKG_INSTALL_LIST_BIN="python3 python3-dev libc-dev linux-headers"
	PKG_INSTALL_LIST_NODEJS=""
	CI_DOCKERFILE_RUN_COMMAND="apk add --update --no-cache gcc python3 python3-dev libc-dev linux-headers \&\& python3 -m ensurepip \&\& pip3 install --no-cache --upgrade pip setuptools pipenv \&\& pipenv install"
	...
	PKG_INSTALL_LIST_NODEJS=""
	CI_DOCKERFILE_RUN_COMMAND="pip3 install pipenv \&\& pipenv install"
...
#
PACKAGEJSON_FILEPATH="${SRCTOP}/k2hr3_osnl/__init__.py"
...
get_repository_package_version()
{
	...
	if ! REPOSITORY_PACKAGE_VERSION="$(grep '^__version__' ${PACKAGEJSON_FILEPATH}| sed -e "s/__version__ = '//g" -e "s/'//g" | tr -d '\n')"; then
...
#	CI_DOCKERFILE_RUN_COMMAND		: install package lis for python
#
custom_dockerfile_conversion()
{
	if [ -z "$1" ] || [ ! -f "$1" ]; then
